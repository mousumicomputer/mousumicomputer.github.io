    ...
    function getDailyClosingStatementData(fromDate, toDate) {
        ...
        if (!isRange && closedSnap) {
            return {
                ...
                reportTime: closedSnap.closing_time || "--:--",
                reportId: closedSnap.report_id || `DCR-${Date.now()}`,
                refId: `REF-HISTORICAL`,
                openingCapital: closedSnap.opening_capital || 0,
                income: closedSnap.income || 0,
                summary: {
                    totalNetBalance: closedSnap.actual_closing || 0
                },
                dueSummary: {
                    todayDilam: closedSnap.total_dilam || totalDilam,
                    todayPelam: closedSnap.total_pelam || totalPelam,
                ...
                container.innerHTML = `
                    <div class="dcr-preview-doc">
                        <div class="dcr-preview-header">
                            <div>Date: ${data.reportDate}<br>Time: ${data.reportTime}</div>
                            <div class="dcr-preview-header-center">
                                <h2>MOUSUMI COMPUTER</h2>
                                <h4>DAILY CLOSING STATEMENT (HISTORICAL)</h4>
                            </div>
                            <div style="text-align:right;">ID: ${data.reportId}</div>
                        </div>
                        <div class="dcr-sec-box">
                            <div class="dcr-sec-bar">FINANCIAL SUMMARY (সংরক্ষিত ক্লোজিং রেকর্ড)</div>
                            <table class="dcr-sec-table">
                                <tr><td>Opening Capital (প্রারম্ভিক মূলধন)</td><td style="text-align:right; font-weight:bold;">৳ ${toEnMoney(data.openingCapital)}</td></tr>
                                <tr><td>Today's Total Dilam (-) [বাকী দেওয়া]</td><td style="text-align:right; color:#dc2626; font-weight:bold;">৳ ${toEnMoney(data.dueSummary.todayDilam)}</td></tr>
                                <tr><td>Today's Total Pelam (+) [আদায় নেওয়া]</td><td style="text-align:right; color:#16a34a; font-weight:bold;">৳ ${toEnMoney(data.dueSummary.todayPelam)}</td></tr>
                                <tr class="total-row" style="background:#f8fafc;">
                                    <td>ACTUAL CLOSING CAPITAL (সমাপ্তি মোট সম্পদ)</td>
                                    <td style="text-align:right; font-weight:bold; font-size:13px; color:#4f46e5;">৳ ${toEnMoney(data.summary.totalNetBalance)}</td>
                                </tr>
                                <tr class="total-row" style="background:#ecfdf5;">
                                    <td>TODAY'S NET PROFIT / COMMISSION</td>
                                    <td style="text-align:right; font-weight:bold; color:#047857;">৳ ${toEnMoney(data.income)}</td>
                                </tr>
                            </table>
                        </div>
                        <div class="dcr-sec-box">
                            <div class="dcr-sec-bar">CUSTOMER DUE SUMMARY</div>
                            <table class="dcr-sec-table">
                                <tr>
                                    <td>Total Customer Outstanding Due (সর্বমোট পাওনা)</td>
                                    <td style="text-align:right; color:#b91c1c; font-weight:bold;">৳ ${toEnMoney(data.dueSummary.totalCustomerDue)}</td>
                                </tr>
                            </table>
                        </div>
                    </div>
...
<body>
    ...
    <\/script>
        ...
        if (rptType === 'daily_closing') {
            ...
            if (!data.found) {
                alert(`${data.reportDate} তারিখে কোনো ডেইলি ক্লোজিং সম্পন্ন করা হয়নি!`);
                return;
            }
...
<body>
    ...
    <div class="section-block">
        <div class="section-bar">FINANCIAL SUMMARY (HISTORICAL CLOSING)</div>
        <table class="statement-table">
            <tr><td>Opening Capital (প্রারম্ভিক মূলধন)</td><td style="text-align:right; font-weight:bold;">৳ ${toEnMoney(data.openingCapital)}</td></tr>
            <tr><td>Today's Total Dilam (-) [বাকী দেওয়া]</td><td style="text-align:right; color:#b91c1c; font-weight:bold;">৳ ${toEnMoney(data.dueSummary.todayDilam)}</td></tr>
            <tr><td>Today's Total Pelam (+) [আদায় নেওয়া]</td><td style="text-align:right; color:#15803d; font-weight:bold;">৳ ${toEnMoney(data.dueSummary.todayPelam)}</td></tr>
            <tr class="total-row" style="background:#f9fafb;">
                <td>ACTUAL CLOSING CAPITAL (মোট সম্পদ)</td>
                <td style="text-align:right; font-weight:bold; font-size:12px;">৳ ${toEnMoney(data.summary.totalNetBalance)}</td>
            </tr>
            <tr class="total-row" style="background:#ecfdf5;">
                <td>TODAY'S NET PROFIT / COMMISSION</td>
                <td style="text-align:right; font-weight:bold; color:#047857;">৳ ${toEnMoney(data.income)}</td>
